...
**Phase 1 (Supabase foundation) — schema drafted, not applied.** `supabase/migrations/20260912230000_init_foundation.sql` covers exactly the Phase 1 build list (profiles, countries, entities, user_roles, campaigns, audit_logs) with RLS enabled from the first migration and every design choice commented inline (e.g. why `profiles` has no client-writable UPDATE policy). `supabase/seed.sql` adds synthetic country/campaign rows only — no employee data. **This is UNVERIFIED**, not "Phase 1 complete": this machine has no Docker, so `supabase start`/`db reset`/`db lint` can't run locally (confirmed — `db lint` failed with connection refused), and no live ITM@15 Supabase project exists yet to push to (blocker 3). Do not treat Phase 1 as done until this migration has actually been applied somewhere and the two Phase 1 acceptance criteria are demonstrated: the database rebuilds cleanly from migrations, and an anonymous browser cannot read private player data.
