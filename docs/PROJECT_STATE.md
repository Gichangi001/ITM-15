...
1. **Run `claude /mcp`** (interactive, in a real terminal — not something I can trigger) to authenticate the now-rescoped `supabase` MCP server against whichever account owns `ysjjgzakswaohmnaowmv`. Once done, tell me and I'll apply `supabase/migrations/20260912230000_init_foundation.sql` via that connection.
