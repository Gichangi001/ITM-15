...
| Unit tests | **Pass** | `pnpm test` (Vitest) — 3 test files, 7 tests (`src/lib/env.test.ts`, `src/lib/env.server.test.ts`, `src/wally/rendering/assets.test.ts`), all passing. |
...
| Visual QA | **Partial pass** | Homepage checked with Playwright screenshots at mobile (390×844) and desktop (1280×800) after adding the Wally mascot image — both render cleanly, sent to the user. Not a full visual-QA pass (no empty/loading/error states to check yet on this one static page). |
