...
_Last updated: 2026-09-13, Phase 3 (onboarding) built and verified live end-to-end against the real project, including a fix for a real live-routing bug found by that verification (see `docs/PROJECT_STATE.md`)._
...
| E2E tests | **Pass — live, manual (not yet a committed automated suite).** Full Phase 2 flow (see prior entry, unchanged) plus, this session, the full Phase 3 onboarding flow against a synthetic `onboarding.test@itm15.test` account: temp-password login → `/first-login` → password change → `/onboarding` (not `/play` — this required a real bugfix, see `PROJECT_STATE.md`) → form submission with real seeded countries → `/play` → re-visiting `/onboarding` bounces away → unauthenticated visit bounces to `/login`. Test account created via service-role key and deleted after verification, same pattern as Phase 2. Scripts remain scratchpad-only, not committed to `tests/e2e/` — same disclosed gap as Phase 2. |
